    ...
    try:
        with sf.SoundFile(file_path, 'r') as audio_file:
            ...
            if np.any(high_freq_mask):
                ...
                if high_freq_ratio > 0.3:  # Increased threshold - natural sounds can have some high freq
...
def create_test_audio_files():
    ...
    t = np.linspace(0, duration, int(sample_rate * duration))
      # 1. Clean rain-like sound (filtered brown noise)
    clean_rain = np.random.normal(0, 0.3, len(t))
    
    # Apply more aggressive filtering to create realistic rain sound
    # Multiple passes of simple filtering
    for _ in range(5):  # More filtering passes
        for i in range(1, len(clean_rain)):
            clean_rain[i] = 0.8 * clean_rain[i] + 0.2 * clean_rain[i-1]
    
    # Additional low-pass effect to reduce high frequencies
    for i in range(2, len(clean_rain)):
        clean_rain[i] = 0.5 * clean_rain[i] + 0.3 * clean_rain[i-1] + 0.2 * clean_rain[i-2]
